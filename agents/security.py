...
def get_client():
    ...
    if not api_key:
        raise RuntimeError(
            "GEMINI_API_KEY is required to run the Security Agent."
        )

    return genai.Client(api_key=api_key)


def review_security_changes(
    changes: list[dict]
) -> list[ReviewFinding]:
...
Identify ONLY genuine security vulnerabilities.
...
- For every genuine vulnerability, return one finding.
    ...
    for attempt in range(3):
        try:
            ...
            if getattr(exc, "status_code", None) == 429:
                print(
                    "Security Agent quota exhausted. "
            ...
            if attempt == 2:
                print(
                    "Security Agent unavailable after 3 attempts. "
                    "Returning no findings for this run."
                )
                return []
            ...
            wait_time = 2 ** attempt
            print(
                f"Security Agent retrying in {wait_time}s..."
            )
